...
async function getManifest() {
  try {
    ...
    // Cache-bust: the CDN can briefly serve a stale copy right after a
    // write even with cache: "no-store" (that only governs the local
    // fetch cache, not edge propagation).
    const res = await fetch(`${info.url}?v=${Date.now()}`, { cache: "no-store" });
